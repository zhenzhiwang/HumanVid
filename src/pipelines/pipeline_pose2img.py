...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        camera_embedding = camera_embedding.to(device=device, dtype=self.camera_pose_encoder.dtype).unsqueeze(2)
        ...
        camera_embedding_feature = torch.cat([camera_embedding_feature] * 2) if do_classifier_free_guidance else camera_embedding_feature
        #print("camera_embedding", camera_embedding_feature.shape)
        #print("pose_fea", pose_fea.shape)
        cond_fea = pose_fea + camera_embedding_feature
