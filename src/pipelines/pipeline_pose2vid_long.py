...
class Pose2VideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #print("pose_fea", pose_fea.shape) 
        # prepare camera embedding
        camera_embedding = camera_embedding.to(device=device, dtype=self.camera_pose_encoder.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for context in global_context:
                    ...
                    cur_pose_cond = torch.cat([pose_cond_tensor[:, :, c] for c in context]).to(device=device, dtype=self.pose_guider.dtype)
                    pose_fea = self.pose_guider(cur_pose_cond)
                    latent_pose_input = pose_fea.repeat(2 if do_classifier_free_guidance else 1, 1, 1, 1, 1)
                    #print("latent_pose_input", latent_pose_input.shape)
                    #print("latent_model_input", latent_model_input.shape)
                    #print("context", context)

                    cur_camera_embedding = torch.cat([camera_embedding[:, :, c] for c in context])
                    #print("cur_camera_embedding", cur_camera_embedding.shape)
                    camera_b = cur_camera_embedding.shape[0]
                    cur_camera_embedding = self.camera_pose_encoder(cur_camera_embedding)[0]      # bf c h w
                    cur_camera_embedding = rearrange(cur_camera_embedding, '(b f) c h w -> b c f h w', b=camera_b)

                    latent_camera_input = cur_camera_embedding.repeat(2 if do_classifier_free_guidance else 1, 1, 1, 1, 1)

                    #print("latent_camera_input", latent_camera_input.shape)
                    pred = self.denoising_unet(
                        latent_model_input,
                        t,
                        encoder_hidden_states=encoder_hidden_states[:b],
                        pose_cond_fea=latent_pose_input + latent_camera_input,
                        return_dict=False,
                    )[0]
